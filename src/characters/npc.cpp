...
void NPC::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("_on_area_2d_body_entered", "body"),
                         &NPC::_on_area_2d_body_entered);
    ClassDB::bind_method(D_METHOD("_on_area_2d_body_exited", "body"),
                         &NPC::_on_area_2d_body_exited);
}
...
void NPC::_process(double_t delta) {
    if (can_talk && Input::get_singleton()->is_action_just_pressed("iteract")) {
        if (ARPG* arpg = Object::cast_to<ARPG>(get_tree()->get_current_scene())) {
            arpg->emit_signal("show_dialog",
                              vformat("%s:  --ola amigo voce e um amigo!!!", get_display_name()));
        }
    }
}
...
Ref<Resource> NPC::get_resource() const { return resource; }

void NPC::set_resource(Ref<Resource> p_resource) { resource = p_resource; }
