...
void Character::set_animation(const String &p_anim_name) {
	// Build the animation name based on the character's look direction (e.g., "run_left", "idle_right", etc.)
	auto new_animation_name = StringName(vformat("%s_%s", p_anim_name, get_look_direction()));

	// Ensure that at least one animation system (AnimationPlayer or AnimatedSprite2D) is available
	if (!animation_player && !animated_sprite) {
		UtilityFunctions::printerr("For use set animation you need setup an AnimationPlayer or AnimatedSprite2D node ");
		return;
	}

	bool has_animation = false; // Tracks whether the requested animation exists

	// Check if the AnimationPlayer has the requested animation
	if (animation_player) {
		if (animation_player->has_animation(new_animation_name)) {
			// Store the base animation name (without direction) for internal reference
	...
	if (animated_sprite) {
		...
		// Defensive check to ensure sprite_frames is loaded and valid
		if (!sprite_frames.is_null() && sprite_frames.is_valid()) {
			// If the animation exists, update the current animation reference
			if (sprite_frames->has_animation(new_animation_name)) {
				current_animation = p_anim_name;
				has_animation = true;
			}
		}

		// If animation was not found in either system, print an error
		if (!has_animation) {
...
void Character::update_animation() {
	...
	if (!animated_sprite && !animation_player) {
		return;
	}

	const StringName new_animation_name = StringName(vformat("%s_%s", current_animation, get_look_direction()));
	...
	if (animated_sprite) {
		...
		if (sprite_frames.is_valid()) {
			if (animated_sprite->get_animation() != new_animation_name) {
				if (sprite_frames->has_animation(new_animation_name)) {
					animated_sprite->play(new_animation_name);
				} else {
					UtilityFunctions::printerr(vformat("AnimatedSprite2D: Animation '%s' not found in %s", new_animation_name, get_name()));
				}
			}
		}
	}
	...
	if (animation_player) {
		if (animation_player->get_current_animation() != new_animation_name) {
			if (animation_player->has_animation(new_animation_name)) {
				...
				UtilityFunctions::printerr(vformat("AnimationPlayer: Animation '%s' not found in", new_animation_name, get_name()));
			}
		}
	}
}
...
void Character::_notification(int p_what) {
	switch (p_what) {
		...
		case NOTIFICATION_PROCESS: {
			...
			if (commands.is_empty()) {
				return; // nada a fazer sem comando
			}
...
PackedStringArray Character::_get_configuration_warnings() const {
	...
	if (!animation_player && !animated_sprite) {
		warnings.append(
				"This node requires an AnimationPlayer or (and) AnimatedSprite2D as a child node for animations to work "
				"correctly.\n"
