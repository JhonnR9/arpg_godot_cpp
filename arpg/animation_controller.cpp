...
/*
 * Automatically captures child nodes of type AnimatedSprite2D and AnimationPlayer
 * as soon as they are added to this node.
 *
 * Godot emits NOTIFICATION_CHILD_ORDER_CHANGED every time a child is added,
 * whether during scene loading (instancing nodes from the .tscn file)
 * or at runtime via add_child().
 *
 * Since the parent node (AnimationController) is always instantiated and inserted
 * into the SceneTree before its children,
 * it always receives this notification for each added child.
 *
 * Therefore, capturing the child using the index get_child_count() - 1 within this notification
 * is safe and guarantees that the newly added node is captured immediately.
 *
 * This eliminates the need for an additional scan of existing children in _ready(),
 * as no child can exist in the SceneTree without the parent having been notified of its addition.
 */
